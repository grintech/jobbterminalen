...
const CompaniesSlider = () => {
  ...
  useEffect(() => {
    const fetchCompanies = async () => {
      try {
        ...
        if (error.response) {
          setError(`API Error: ${error.response.data.message || 'Unknown error from server'}`);
    ...
    if (userId) {
      ...
      .then((response) => {
        if (response.data.type === 'success') {
          const savedCompanyIds = response.data.data.map((company) => company.company_id);
          setLikedCards(savedCompanyIds.reduce((acc, id) => ({ ...acc, [id]: true }), {}));
        }
      })
      .catch((error) => {
  ...
  const toggleLike = async (companyId) => {
    ...
    try {
      ...
      if (response.data.type === 'success') {
        ...
        const actionMessage = likedCards[companyId] ? 'Company Unsaved' : 'Company Saved';
  ...
  return (
    <div className="top_companies container py-5">
      <h4 className="mb-3 text-center text-md-start">Recently Added Companies</h4>
                  ...
                  <div className="card h-100">
                    <div className="card-body text-center">
                      <Link to={`/companies/${company.slug}`}>
                        <img
                          src={`${IMG_URL}/${company.company_profile === 'null' ? '/public/images/frontimg/blank_logo.jpg' : company.company_profile}`}
